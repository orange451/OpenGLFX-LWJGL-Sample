...
public class ScriptRunner extends LuaValue implements Runnable {
	...
	private static final LuaValue C_SCRIPT = LuaValue.valueOf("script");

	static {
		THREAD_POOL = Executors.newScheduledThreadPool(2048, (runnable) -> {
	...
	public void run() {
		...
		try {
			if ( script != null ) {
				LuaEngine.globals.set(C_SCRIPT, script);
