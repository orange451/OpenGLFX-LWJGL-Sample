...
public abstract class DataModel extends LuaDatatype {
	...
	private static final LuaValue C_ANCESTRYCHANGED = LuaValue.valueOf("AncestryChanged");
	...
	public DataModel(String name) {
		...
		this.rawset(C_ANCESTRYCHANGED,	new LuaEvent());
	...
	public String getFullName() {
		...
		LuaValue p = this;
		int i = 0;
		boolean includeDotPrefix = true;
		while ( p != null && p instanceof Instance ) {
			Instance inst = ((Instance)p);
			p = inst.getParent();
			
			String myName = inst.getName();
			boolean hasSpace = myName.contains(" ");
			if ( hasSpace )
				myName = "['" + myName + "']"; 
			
			if ( i > 0 && includeDotPrefix )
				ret = "." + ret;
			ret = myName + ret;
			
			i++;
			includeDotPrefix = !hasSpace;
		}
		return ret;
	}
	...
	private void checkSetParent(LuaValue key, LuaValue oldParent, LuaValue newParent) {
		if ( oldParent.isnil() && newParent.isnil() )
			return;
		...
		if ( key.eq_b(C_PARENT) ) {
			...
			if ( newParent instanceof Instance && !((Instance)newParent).isDestroyed() ) {
				...
				// Fire AncestryChanged event
				ancestryChanged(this, newParInst);
	...
	private void ancestryChanged(DataModel child, DataModel parent) {
		ancestryChangedEvent().fire(child, parent);
		for (Instance descendent : descendents) {
			descendent.ancestryChangedEvent().fire(child, parent);
		}
	}
	...
	/**
	 * The ancestry changed event for the DataModel. AncestryChanged fires immediately when the Parent property of an instance changes. This event fires for this instance and all of its descendants.
	 * @return
	 */
	public LuaEvent ancestryChangedEvent() {
		LuaValue temp = this.rawget(C_ANCESTRYCHANGED);
		return temp.isnil()?null:(LuaEvent)temp;
	}
