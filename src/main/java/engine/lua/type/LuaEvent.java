	...
	public void fire(LuaValue...args) {
		...
		synchronized(connections) {
			...
			for (int i = 0; i < len; i++) {
				...
				if ( temp == null || temp.getFunction() == null )
					continue;

				/*LuaValue coroutine = LuaEngine.globals.get("coroutine");
				if ( !coroutine.isnil() ) {
					LuaThread lt = (LuaThread) coroutine.get("create").call(temp.getFunction());
					coroutine.get("resume").invoke(LuaValue.varargsOf( lt, vargs ));
				}*/
